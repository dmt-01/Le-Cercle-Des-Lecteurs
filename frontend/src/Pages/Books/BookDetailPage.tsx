  ...
  if (!book) {
    return (
      <div className="max-w-5xl mx-auto px-6 py-20 text-center">
        ...
        <Link to="/books" className="text-secondary hover:underline text-sm">
          ...
          <div className="rounded-2xl overflow-hidden aspect-[2/3] bg-beige-medium shadow-md">
            {book.cover_image ? (
              ...
              <div className="w-full h-full bg-gradient-to-br from-secondary to-primary flex items-center justify-center p-6">
                <p className="text-white font-serif italic text-center text-sm">
                  {book.title}
                </p>
              </div>
            )}
                ...
                <div className="flex flex-col gap-1.5 pt-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {book.tags.map((tag) => (
                      <span
                        key={tag.id}
                        className="bg-beige text-primary/60 text-[10px] uppercase tracking-wide px-2 py-0.5 rounded-full"
                      >
                        {tag.name}
                      </span>
                    ))}
                ...
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map((starValue) => (
                    <span
                      key={starValue}
                      className={
                        starValue <= Math.round(book.average_rating!)
                          ? "text-gold text-lg"
                          : "text-beige-medium text-lg"
                      }
                    >
                      ★
                    </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-2">
                ...
                <div className="bg-white rounded-xl border border-beige-medium p-5">
                  ...
                  <p className="text-sm text-primary/60 leading-relaxed mb-4">
                    Rejoignez nos discussions en direct avec les membres du
                    cercle exclusif.
                  </p>
                  <Link
                    to="/events"
            ...
            <div className="flex flex-col gap-4">
              {reviews.length === 0 && (
                <p className="text-primary/40 text-sm italic">
                  Aucune critique pour ce livre.
                </p>
              )}
              {reviews.map((review, index) => (
                <div
                  key={index}
                  className="bg-white rounded-xl border border-beige-medium p-5 flex flex-col gap-3"
                >
