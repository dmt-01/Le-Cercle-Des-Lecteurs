...
export default class WishlistController extends Controller {
  ...
  async add() {
    try {
      const userId              = this.request.userId!;
      const { bookId, status }  = this.request.body as { bookId: string; status: string };
      await wishlistService.add(userId, bookId, status);
      return this.response.status(200).json({ message: "Livre ajouté à la wishlist" });
    } catch (error: any) {
  ...
  async remove() {
    try {
      ...
      return this.response.status(200).json({ message: "Livre retiré de la wishlist" });
    } catch (error: any) {
      const status = error instanceof AppError ? error.statusCode : 500;
      return this.response.status(status).json({ message: error.message });
    }
